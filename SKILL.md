...
description: >
  ...
  创建飞书云文档 → AI初筛并给出推荐等级 → 向推荐候选人发索要简历消息 → 更新操作记录到文档。
...
> ⚠️ **重要**：不要一次性并行所有请求。opencli 通过 Chrome 扩展通信，大量并发请求会导致数据混乱或重复。建议每批并行 4-5 个，逐批执行。

将候选人 uid 分批，每批运行：

```bash
# 示例：每批 4-5 个并行
opencli boss resume "<uid1>" -f json &
opencli boss resume "<uid2>" -f json &
opencli boss resume "<uid3>" -f json &
opencli boss resume "<uid4>" -f json &
wait  # 等待本批完成再执行下一批
```
...
| 等级 | 含义 | 行动 |
|------|------|------|
| ⭐⭐⭐⭐⭐ 强推 | 经验高度匹配，薪资合理 | 发索要简历消息 |
| ⭐⭐⭐⭐ 推荐 | 基本匹配，有明显亮点 | 发索要简历消息 |
| ⭐⭐⭐ 可约 | 部分匹配，需面试确认 | 可选择是否发消息 |
| ⭐⭐ 观望 | 匹配度弱，背景存疑 | 暂不操作 |
| ⭐ 不推荐 | 明显不合适（错位/经验不足/虚高） | 标记不合适 |
| ❌ 排除 | 完全不相关（投错职位等） | 标记不合适 |
...
### Step 8：向推荐候选人发送索要简历消息

在发送前，先验证 Cookie 是否仍然有效（可用 `opencli boss chatlist 2>&1 | head -3` 快速检查）。

若 Cookie 过期，提示用户重新登录后再发送。

对 ⭐⭐⭐⭐ 和 ⭐⭐⭐⭐⭐ 的候选人，**并行发送**：

```bash
opencli boss send "<uid>" "您好！非常感谢您的关注，我们对您的背景很感兴趣，能否麻烦您发一份详细简历过来？期待进一步了解您的经历，谢谢！"
```
...
```bash
opencli boss mark "<uid>" --label "不合适"
```

> ⚠️ **已知限制**：`mark` 命令只对「推荐候选人」列表有效，对聊天列表中的候选人会报 `未知的非法参数 (code=2)`。若失败，记录在操作记录中，提示用户手动在 Boss直聘 APP/网页端操作。

---

### Step 10：将操作记录追加到飞书文档

将本次操作结果追加到文档末尾，包含：
- ✅ 成功发送索要简历消息的候选人列表（含职位）
- ⚠️ 需手动标记不合适的候选人列表（含原因）
...
| 并行获取简历出现重复/错误数据 | opencli 并发限制 | 改为分批执行（每批 4-5 个），等待 `wait` 后再下一批 |
| `mark` 报 `未知的非法参数` | opencli 限制，不支持聊天列表标记 | 记录并提示用户手动操作 |
