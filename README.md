...
| 等级 | 含义 | 自动动作 |
|------|------|----------|
| ⭐⭐⭐⭐⭐ 强推 | 经验高度匹配，薪资合理 | 自动发索要简历消息 |
| ⭐⭐⭐⭐ 推荐 | 基本匹配，有明显亮点 | 自动发索要简历消息 |
| ⭐⭐⭐ 可约 | 部分匹配，需面试确认 | 可选 |
| ⭐⭐ 观望 | 匹配度弱 | 不操作 |
| ⭐ 不推荐 | 明显不合适 | 尝试标记不合适 |
| ❌ 排除 | 完全不相关 | 尝试标记不合适 |
...
**Q: `mark` 命令报 `未知的非法参数`**
A: 已知限制，`opencli boss mark` 只对「推荐候选人」列表有效，聊天列表中的候选人需手动在 Boss直聘 APP/网页端标记。

**Q: 并行获取简历出现重复/错误数据**
A: Skill 已改为分批执行（每批 4-5 个）。如仍有问题，可让 Claude 改为逐个顺序获取。
